const daphO = ["https://static-cdn.jtvnw.net/emoticons/v1/302071866/3.0", //daphBlind
    ...
    const move = {
        ...
        right: function () {
            marquee.animate(
                ...
                {
                    duration: ((container.offsetWidth / settings.speed) * 1000), easing: "linear"
                }
            ).onfinish = function () {
                settings.bumpEdge();
                move.left()
        ...
        left: function () {
            marquee.animate(
                [{ left: `${(container.offsetWidth - marquee.offsetWidth)}px` }, { left: 0 }],
                {
                    duration: ((container.offsetWidth / settings.speed) * 1000), easing: "linear"
                }
            ).onfinish = function () {
...
dvd({
    speed: 300,
    bumpEdge: function () {
        document.querySelector(".daphO").src = daphO[Math.floor(Math.random() * daphO.length)];
    }
})
